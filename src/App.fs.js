...
export let autoScrollEnabled = createAtom(false);
...
export function render(state) {
    ...
    if (FSharpMap__ContainsKey(state.Displays, "right") && FSharpMap__ContainsKey(state.Displays, "left")) {
    ...
    try {
        while (enumerator_1["System.Collections.IEnumerator.MoveNext"]()) {
            ...
            if (c.tagName === "A") {
                c.onclick = ((_arg_3) => {
                    autoScrollEnabled(true, true);
                    toConsole(printf("going awaaaaay"));
                });
            }
        }
    }
